...
def data_frame_modele(
    ...
    Fonction permettant de convertir des fichiers de données au format json
    contenant des objets de la classe `Voiture` en dataframe
    ...
    Deux dictionnaire crée par la fonction `data_frame_sklearn` permettant
    l'implémentation dans sklearn.
    """
...
def assemblage_donnees(
    ...
    Fonction permettant l'assemblage des différents fichiers json
    de données concernant les voitures (`Voiture`) pour créer un
...
def _reconstitution_fichier(contenu: str) -> list[Voiture]:
    ...
    Fonction permettant de reconstituer une liste d'objets de classe `Voiture`
    à partir de la chaîne de caractères extraite
    du fichier json.
...
def data_frame_pandas(fichier: str) -> pd.core.frame.DataFrame:
    """
    # Description

    Fonction permettant de constituer un dataframe du module `pandas`
    à partir d'un fichier json contenant différents objets de la
    classe `Voiture`.
...
def data_frame_imputation(df: pd.core.frame.DataFrame) -> pd.core.frame.DataFrame:
    ...
    Les données numériques sont imputées selon la moyenne, les données catégorielles
    selon la valeur majoritaire.
...
def data_frame_dummies(df: pd.core.frame.DataFrame) -> pd.core.frame.DataFrame:
    ...
    On notera la création d'un certain nombre de colonnes
    par la fonction pour la création de ces variables.
    ...
    Un dataframe du module `pandas` avec des variables dichotomique pour
    remplacer les catégorielles.
    """
...
class DataFrameImputer(TransformerMixin):
    """
    # Description
    Classe permettant de faciliter l'imputation des données
    dans la fonction `data_frame_imputation`.
    """
