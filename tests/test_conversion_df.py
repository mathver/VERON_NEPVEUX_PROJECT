...
def test_assemblage():
   ...
   assert pd.testing.assert_frame_equal(realite,predit) == None
   
   
def test_ouverture_fichier():
   realite = conv._ouverture_fichier("./veron_nepveux_project/donnees_peugeot.json")[1:398]
   predit = '{"marque":"Peugeot","modele":"308","carburant":"Diesel","prix":20290,"kilometrage":75967,"garantie_kilometrage":"non garanti","boite_de_vitesse":"Automatique","transmission":2,"couleur":"Blanc","garantie":"SPOTICAR PREMIUM","date_mise_circulation":2019,"puissance":130,"silhouette":"Berline","nb_places":5,"utilisation_prec":"Ex-Loueur","puissance_fiscale":6,"critair":2,"ptac":1890,"nb_portes":5}'
   assert predit == realite


def test_reconstitution_fichier():
   realite = conv._reconstitution_fichier('[{"marque":"Peugeot","modele":"308","carburant":"Diesel","prix":20290,"kilometrage":75967,"garantie_kilometrage":"non garanti","boite_de_vitesse":"Automatique","transmission":2,"couleur":"Blanc","garantie":"SPOTICAR PREMIUM","date_mise_circulation":2019,"puissance":130,"silhouette":"Berline","nb_places":5,"utilisation_prec":"Ex-Loueur","puissance_fiscale":6,"critair":2,"ptac":1890,"nb_portes":5}]')
   predit = [Voiture(marque='Peugeot', modele='308', carburant='Diesel', prix=20290, kilometrage=75967, garantie_kilometrage='non garanti', boite_de_vitesse='Automatique', transmission=2, couleur='Blanc', garantie='SPOTICAR PREMIUM', date_mise_circulation=2019, puissance=130, silhouette='Berline', nb_places=5, utilisation_prec='Ex-Loueur', puissance_fiscale=6, critair=2, ptac=1890, nb_portes=5)]
   assert realite == predit
   


def test_data_frame_pandas():
   realite = conv.data_frame_pandas("./veron_nepveux_project/donnees_peugeot.json").iloc[0:2]
   predit = pd.DataFrame.from_dict({'marque': {0: 'Peugeot', 1: 'Peugeot'},
   'modele': {0: '308', 1: '3008'},
   ...
   assert pd.testing.assert_frame_equal(predit,realite) == None
   
def test_data_frame_imputation():
   df = conv.data_frame_pandas("./veron_nepveux_project/donnees_peugeot.json").iloc[0:2]
...
def test_data_frame_dummies():
   ...
   predit = pd.DataFrame.from_dict({'prix': {0: 20290, 1: 31900},
   'kilometrage': {0: 75967, 1: 29066},
   'date_mise_circulation': {0: 2019, 1: 2021},
   'puissance': {0: 130.0, 1: 130.0},
   'nb_places': {0: 5.0, 1: 5.0},
   ...
   'Ex-Loueur': {0: 1, 1: 1}})
   assert  pd.testing.assert_frame_equal(predit,realite,check_dtype=False) == None

def test_data_frame_sklearn():
   df = conv.data_frame_pandas("./veron_nepveux_project/donnees_peugeot.json").iloc[0:2]
   realite_X,realite_y = conv.data_frame_sklearn(df)
   predit_x = np.array([['Peugeot', '308', 'Diesel', 75967, 'non garanti', 'Automatique',
         2.0, 'Blanc', 'SPOTICAR PREMIUM', 2019, 130.0, 'Berline', 5.0,
         'Ex-Loueur', 6.0, 2.0, 1890.0, 5.0],
         ['Peugeot', '3008', 'Diesel', 29066, 'non garanti', 'Manuelle',
         2.0, 'Gris', 'SPOTICAR PREMIUM', 2021, 130.0, 'SUV-4x4', 5.0,
         'Ex-Loueur', 7.0, 2.0, 1875.0, 5.0]], dtype=object) 
